...
static int ublk_ctrl_add_dev(struct io_uring_cmd *cmd)
{
	...
	if (header->len < sizeof(info) || !header->addr) {
        if (header->len < sizeof(info)) printk("header->len < sizeof(info)\n");
        if (!header->addr) printk("!header->addr\n");
        return -EINVAL;
    }
	if (header->queue_id != (u16)-1) {
        printk("%s: queue_id is wrong %x\n",
		...
		unsigned int issue_flags)
{
	...
	struct ublk_device *ub = NULL;
	u32 cmd_op = cmd->cmd_op;
    printk("prota edo , header->dev_id = %d\n", header->dev_id);
	int ret = -EINVAL;
