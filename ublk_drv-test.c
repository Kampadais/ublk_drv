...
//	printk("%s: complete: op %d, qid %d tag %d io_flags %x addr %llx\n",
...
//			printk("%s: need get data. op %d, qid %d tag %d io_flags %x\n",
...
//		printk("%s: update iod->addr: op %d, qid %d tag %d io_flags %x addr %llx\n",
//				__func__, io->cmd->cmd_op, ubq->q_id, req->tag, io->flags,
//				ublk_get_iod(ubq, req->tag)->addr);
	}
...
{
//    printk("%s: quiesce ub: dev_id %d state %s\n",
...
//    printk("%s: unquiesce ub: dev_id %d state %s\n",
//			__func__, ub->dev_info.dev_id,
//			ub->dev_info.state == UBLK_S_DEV_LIVE ?
//			"LIVE" : "QUIESCED");
...
//    printk("%s: received: cmd op %d queue %d tag %d result %d\n",
...
static int ublk_ctrl_start_dev(struct ublk_device *ub, struct io_uring_cmd *cmd)
{
   ...
   // printk("ublksrv_pid = %d\n", ublksrv_pid);
	if (ublksrv_pid <= 0) {
    //    printk("ublksrv_pid <= 0\n");
        return -EINVAL;
    }

	if (wait_for_completion_interruptible(&ub->completion) != 0) {
  //      printk("wait_for_completion_interruptible(&ub->completion) != 0\n");
        return -EINTR;
    }
  //  printk("wait_for_completion_interruptible(&ub->completion) == 0\n");
...
{
 //   printk("%s: dev id %d flags %llx\n", __func__,
//			info->dev_id, info->flags);
 //   printk("\t nr_hw_queues %d queue_depth %d\n",
...
//    printk("%s: cmd_op %x, dev id %d qid %d data %llx buf %llx len %u\n",
...
//    printk("%s: start recovery for dev id %d.\n", __func__, header->dev_id);
	for (i = 0; i < ub->dev_info.nr_hw_queues; i++)
...
//    printk("%s: Waiting for new ubq_daemons(nr: %d) are ready, dev id %d...\n",
//			__func__, ub->dev_info.nr_hw_queues, header->dev_id);
//	/* wait until new ubq_daemon sending all FETCH_REQ */
	if (wait_for_completion_interruptible(&ub->completion))
		return -EINTR;

//    printk("%s: All new ubq_daemons(nr: %d) are ready, dev id %d\n",
//			__func__, ub->dev_info.nr_hw_queues, header->dev_id);
	...
	ub->dev_info.ublksrv_pid = ublksrv_pid;
//    printk("%s: new ublksrv_pid %d, dev id %d\n",
//			__func__, ublksrv_pid, header->dev_id);
	blk_mq_unquiesce_queue(ub->ub_disk->queue);
//    printk("%s: queue unquiesced, dev id %d.\n",
//			__func__, header->dev_id);
	blk_mq_kick_requeue_list(ub->ub_disk->queue);
	ub->dev_info.state = UBLK_S_DEV_LIVE;
	ret = 0;
...
//    printk("%s: dev id %d cmd_op %x uid %d gid %d path %s ret %d\n",
		...
		unsigned int issue_flags)
{
    ...
    //printk("cmd_op %d",cmd_op);

	if (issue_flags & IO_URING_F_NONBLOCK){
  //      printk("IO_URING_F_NONBLOCK");
        return -EAGAIN;
    }
	...
	switch (_IOC_NR(cmd_op)) {
	case UBLK_CMD_START_DEV:
        printk("UBLK_CMD_START_DEV");
		ret = ublk_ctrl_start_dev(ub, cmd);
		break;
	case UBLK_CMD_STOP_DEV:
        printk("UBLK_CMD_STOP_DEV");
		ret = ublk_ctrl_stop_dev(ub);
		break;
	case UBLK_CMD_GET_DEV_INFO:
	case UBLK_CMD_GET_DEV_INFO2:
        printk("UBLK_CMD_GET_DEV_INFO");
		ret = ublk_ctrl_get_dev_info(ub, cmd);
		break;
	case UBLK_CMD_ADD_DEV:
        printk("UBLK_CMD_ADD_DEV");
		ret = ublk_ctrl_add_dev(cmd);
		break;
	case UBLK_CMD_DEL_DEV:
        printk("UBLK_CMD_DEL_DEV");
		ret = ublk_ctrl_del_dev(&ub);
		break;
	case UBLK_CMD_GET_QUEUE_AFFINITY:
        printk("UBLK_CMD_GET_QUEUE_AFFINITY");
		ret = ublk_ctrl_get_queue_affinity(ub, cmd);
		break;
	case UBLK_CMD_GET_PARAMS:
        printk("UBLK_CMD_GET_PARAMS");
		ret = ublk_ctrl_get_params(ub, cmd);
		break;
	case UBLK_CMD_SET_PARAMS:
		printk("UBLK_CMD_SET_PARAMS");
 ...
 out:
	io_uring_cmd_done(cmd, ret, 0, issue_flags);
//    printk("%s: cmd done ret %d cmd_op %x, dev id %d qid %d\n",
